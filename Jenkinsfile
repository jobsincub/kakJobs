...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3602
        NAMESPACE = "picassonova-online"
        REGISTRY_HOSTNAME = "itincback"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "picasso-front"
        DEPLOYMENT_NAME = "picasso-front-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'picassonova-online') {
